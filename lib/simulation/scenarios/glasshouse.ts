...
export const glasshouse: ScenarioDefinition = {
  ...
  assistance: { guided: ["Use curl on the known portal to inspect published relationships, or nmap to inspect its services. Requesting the portal's page reveals any form it publishes, including the method and field it expects.", "Use ls, cat, and ps after gaining a session. Files reveal identities and reachable systems; a process's full command line often reveals the configuration file it loads.", "A PostgreSQL service accepts a known identity through psql. You do not need to know the database name in advance: connect with -h and -U only, then use \\l to list databases and \\c to select one, and \\dt / \\d <table> to see what's inside before querying."], operator: "Follow the evidence across application, backup, and database trust boundaries." },
