...
export default function ReplayClient({ scenarioId, actorId }: { scenarioId: string; actorId?: string }) {
  ...
  useEffect(() => {
    ...
    if (!actor) { queueMicrotask(() => setError("Review authorization was not found for this engagement.")); return; }
    void fetch(`/api/sim/replay?scenarioId=${scenarioId}&actorId=${actor}`, { cache: "no-store" })
      .then(async (response) => {
        const data = await response.json();
        if (!response.ok || !data.success) throw new Error(data.error ?? "After-action record unavailable");
        setPayload(data);
      ...
      .catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "After-action record unavailable"));
  }, [actorId, scenarioId]);
  ...
  if (error) return <main className="loading-screen"><div className="boot-mark">ROOT<span>/OS</span></div><p>{error}</p><Link className="primary-button" href="/">Return to Operations Desk</Link></main>;
  if (!payload) return <main className="loading-screen"><div className="boot-mark">ROOT<span>/OS</span></div><p>Loading correlated operation records…</p></main>;
  ...
  return <RootChrome context="replay" active="replay" tone="truth" title={`${payload.operation.presentation.caseId} · Review record`} privilege="ANALYST">
    <div className="review-application">
      <nav className="review-nav"><button className="active">Summary</button><button>Timeline</button><button>Operator Activity</button><button>Defensive Response</button><button>Correlated View</button><button>System Events</button><button>Evidence</button><button>Notes</button><button>Review Findings</button></nav>
    <div className="replay-layout">
      <header className="replay-heading"><div><h1>{payload.operation.name} <b>— Operation Reconstruction</b></h1><p>Outcome: {payload.progression.won ? "Primary tasking completed" : "Tasking incomplete"} · {payload.availability.percent}% business availability</p></div><span>{payload.operation.organization} · {payload.operation.presentation.caseId}</span></header>
      <div className="lens-tabs">{(["RED", "BLUE", "TRUTH"] as ReplayLens[]).map((item) => <button key={item} onClick={() => selectLens(item)} className={`${lens === item ? "active" : ""} ${item.toLowerCase()}`}><strong>{item === "TRUTH" ? "Correlated View" : item === "RED" ? "Operator Activity" : "Defensive Response"}</strong><span>{item === "RED" ? "Records available to the operator" : item === "BLUE" ? "Monitoring and response records" : "Verified reconstruction"}</span></button>)}</div>

      <section className="panel replay-timeline-panel">
        <div className="panel-title"><span>Event Timeline <b>— {lens === "TRUTH" ? "Correlated View" : lens === "RED" ? "Operator Activity" : "Defensive Response"}</b></span><button className="replay-control" onClick={() => { if (selectedIndex >= visible.length - 1) setSelectedIndex(0); setPlaying((value) => !value); }}>{playing ? "Ⅱ Pause" : "▶ Play"}</button></div>
      ...
      <aside className="replay-side">
        <section className="panel operation-summary"><div className="panel-title"><span>Engagement Outcome</span><strong>{payload.progression.won ? "Tasking Complete" : "Needs Review"}</strong></div><h2>{payload.operation.name}</h2><p>{payload.operation.organization}<br />Tasking: {payload.operation.objective}</p><dl><dt>ROUTE IDENTIFIED</dt><dd>{payload.operation.availableModes.includes("BLUE") ? payload.summary.route?.name ?? "Unresolved" : "Not applicable"}</dd><dt>DURATION</dt><dd>{formatElapsed(payload.summary.durationMs)}</dd><dt>DETECTION TIME</dt><dd>{formatElapsed(payload.summary.detectionTimeMs)}</dd><dt>CONTAINED</dt><dd>{payload.summary.contained ? "Yes" : "No"}</dd><dt>SESSIONS CREATED</dt><dd>{payload.summary.sessionCount}</dd><dt>PERSISTENCE</dt><dd>{payload.summary.persistenceInstalled ? "Installed" : "None"}</dd><dt>REVIEW STATUS</dt><dd className={payload.progression.won ? "green-text" : "red-text"}>{payload.progression.won ? "Satisfactory" : "Needs Review"}</dd></dl></section>
        <section className="panel path-panel"><div className="panel-title"><span>Observed Access Path <b>— Event Derived</b></span></div><div className="attack-path">{payload.summary.attackPath.map((host, index) => <span key={`${host}-${index}`}>{index > 0 && <i>→</i>}<b>{host}</b></span>)}</div></section>
      </aside>
      ...
      <section className="panel learning-summary"><div className="panel-title">ROUTE COMPARISON / RESPONSE REVIEW</div><div className="data-list">
        ...
        <h2>Unresolved indicators</h2>{payload.missedOpportunities.map((item) => <p key={item.id}>{item.target}: {item.title}. {item.rationale}</p>)}{!payload.missedOpportunities.length && <p>No unreviewed detections.</p>}
        ...
        <h2>Learning evidence</h2>{payload.summary.learningEvidence.map((item) => <p key={item.objectiveId}><strong>{item.label}</strong>: {item.evidence ?? "Authoritative task evidence recorded."} {item.concepts.length ? `Concepts: ${item.concepts.join(", ")}.` : ""}</p>)}{!payload.summary.learningEvidence.length && <p>No learning annotations were recorded.</p>}
        <h2>Competency record</h2><p>{payload.progression.concepts.join(" · ") || "No new competency evidence was recorded."}</p>
      </div></section>
      <section className="panel learning-summary"><div className="panel-title"><span>Review Findings and Evidence Profile</span><span>{payload.summary.evidenceCount} evidence events</span></div><article><h2>{payload.keyDecision.title}</h2><p>{perspective}</p>{payload.summary.route && <p>{payload.summary.route.evidenceProfile.comparison} Recorded footprint: {payload.summary.route.actual.networkEvents} network/web · {payload.summary.route.actual.authenticationEvents} authentication · {payload.summary.route.actual.endpointEvents} endpoint · {payload.summary.route.actual.detectionCount} detections.</p>}<small>{snapshot.objectiveRetrieved ? "Task evidence: complete" : "Task evidence: incomplete"} · Active sessions: {snapshot.activeSessions.length}</small></article><Link className="primary-button" href="/">Return to Operations Desk</Link></section>
    </div>
    </div>
  </RootChrome>;
}
