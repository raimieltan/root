...
export function useOperation(mode: "RED" | "BLUE") {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const definitionId = params.get("operation") ?? "first-shift";
      const requestedAssistance = params.get("assistance") === "OPERATOR" ? "OPERATOR" : "GUIDED";
      const definition = campaign.find((operation) => operation.id === definitionId);
      const operatorAvailable = campaignProgress(readRuns()).operatorModeUnlocked || definition?.assistance.operatorAvailableAtStart;
      const assistance = requestedAssistance === "OPERATOR" && operatorAvailable ? "OPERATOR" : "GUIDED";
